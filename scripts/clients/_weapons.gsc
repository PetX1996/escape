...
{
	player.concussionEndTime = 0;
	player.hasDoneCombat = false;
	player thread watchWeaponUsage();
	player thread watchWeaponReload();
	player thread watchGrenadeUsage();
	//player thread watchWeaponChange();
		
	player.droppedDeathWeapon = undefined;
	player.tookWeaponFrom = [];
		
	player thread updateStowedWeapon();
	
	player thread MonitorChangeWeapons();
}

// callback
MonitorChangeWeapons()
{
	self endon("death");
	...
	while( true )
	{
		...
		if( self CheckActionSlots( newWeapon ) )
		{
			self SwitchToWeapon( last );
			continue;
		}
		
		// call callbacks
		scripts\_events::RunCallback( level, "weaponChange", 1, self, last, newWeapon );
		scripts\_events::RunCallback( self, "weaponChange", 1, last, newWeapon );
		
		last = newWeapon;
	}	
}

CheckActionSlots( newWeapon )
{
	switch( newWeapon )
	{
		case "radar_mp": // as 1 - perk 3
			scripts\_events::RunCallback( level, "perkButton", 1, self );
			scripts\_events::RunCallback( self, "perkButton", 1 );
			return true;
		case "airstrike_mp": // as 3 - equipment
			scripts\_events::RunCallback( level, "equipmentButton", 1, self );
			scripts\_events::RunCallback( self, "equipmentButton", 1 );
			return true;
		case "helicopter_mp": // as 4 - gear
			scripts\_events::RunCallback( level, "gearButton", 1, self );
			scripts\_events::RunCallback( self, "gearButton", 1 );
			return true;
		default:
			return false;
	}
